...
2. `align_mfa.sh` runs Montreal Forced Aligner in `mmcauliffe/montreal-forced-aligner@sha256:1986960fcb5169979630a7efb2576480c587500ab556c9daa66a930f471215b8` Docker with the pinned language models `japanese_mfa`, `english_mfa`, and `mandarin_mfa`. The host cache is mounted at the image's `/mfa` root; no MFA or Qwen runtime is imported by the Electron build.
